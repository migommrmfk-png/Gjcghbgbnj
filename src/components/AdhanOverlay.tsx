...
export default function AdhanOverlay({ prayerName, time, onClose }: { prayerName: string, time: string, onClose: () => void }) {
  ...
  const getMosqueImage = () => {
    switch (prayerName) {
      case "الفجر":
        return "https://images.unsplash.com/photo-1519817914152-2a241f6d54f6?q=80&w=1080&auto=format&fit=crop"; // Dawn
      case "الظهر":
      case "العصر":
        return "https://images.unsplash.com/photo-1564121211835-e88c852648ab?q=80&w=1080&auto=format&fit=crop"; // Day
      case "المغرب":
        return "https://images.unsplash.com/photo-1551041777-ed277b8dd348?q=80&w=1080&auto=format&fit=crop"; // Sunset
      case "العشاء":
        return "https://images.unsplash.com/photo-1507676184212-d0330a15233c?q=80&w=1080&auto=format&fit=crop"; // Night
      default:
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center text-center space-y-8 p-6">
        <motion.div
          ...
          className="w-32 h-32 rounded-full bg-white/10 backdrop-blur-lg border border-white/20 flex items-center justify-center shadow-2xl"
        >
          <Clock size={64} className="text-[var(--color-gold)]" />
        </motion.div>
        ...
        >
          <h1 className="text-6xl font-serif font-bold mb-4 drop-shadow-lg text-[var(--color-gold-light)]">
            {currentTime.toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}
          </h1>
          <h2 className="text-4xl font-serif font-bold drop-shadow-md">حان الآن موعد أذان {prayerName}</h2>
          <p className="text-xl mt-4 opacity-80 font-bold">حسب التوقيت المحلي</p>
        </motion.div>
